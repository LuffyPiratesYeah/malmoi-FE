...
export function DashboardClient({ schedules, isLoading }: DashboardClientProps) {
    ...
    const handleAddQuickTime = () => {
        ...
        addQuickTime({ label: quickTimeForm.label, date: quickTimeForm.date, time: quickTimeForm.time });
    ...
    const handleBookWithQuickTime = async () => {
        ...
        if (new Date(`${bookingDate}T00:00:00`) < new Date(`${today}T00:00:00`)) {
                                ...
                                quickTimes.map((qt) => (
                                    ...
                                    >
                                        <div className="flex flex-col items-start">
                                            <span>{qt.label}</span>
                                            <span className="text-xs font-normal opacity-80">{qt.date ? `${qt.date} ` : ""}{qt.time}</span>
                                        </div>
