        ...
        <main className="mx-auto max-w-7xl px-8 py-12">
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                            ...
                            <>
                                <div className="flex items-center justify-between mb-8">
                                    ...
                                    <div className="flex gap-3">
                                        <Button className="h-11 px-6 text-sm font-bold bg-[#00C2FF] hover:bg-[#00C2FF]/90 text-white shadow-md shadow-blue-200 border-none">
                                ...
                                <div className="border-t border-gray-100 pt-8">
                                    <h4 className="mb-4 font-bold text-gray-900">수업 준비</h4>
                                    <div className="space-y-3">
                                        {["예습 PDF 한 번 읽기", "오늘 회의 상황 메모해오기", "지난 수업 복습 노트 확인"].map((item, i) => (
                                            <label key={i} className="flex items-center gap-3 cursor-pointer group">
                                                ...
                                                />
                                                <span className={cn("text-sm transition-colors", prepChecks[i] ? "text-gray-900 font-medium" : "text-gray-600")}>{item}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                ...
                <div className="space-y-8">
                    {/* To Do */}
                    <Card title="오늘 할 일" subtitle="오늘 수업을 위해 이 정도만 준비해보세요." className="border border-gray-100 shadow-sm">
                    ...
                    {/* Quick Reserve */}
                    <Card title="빠른 수업 예약" subtitle="원하는 시간에 1:1 수업을 빠르게 예약해보세요." className="border border-gray-100 shadow-sm">
                        ...
                        <Link href="/class">
                            <Button className="mt-6 w-full bg-[#00C2FF] hover:bg-[#00C2FF]/90 text-white font-bold h-12" size="lg">
                                이 시간으로 수업 예약하기
                            </Button>
                        </Link>
                    </Card>
            ...
            <div className="mt-8">
                <Card className="border border-gray-100 shadow-sm">
                    <div className="mb-6 flex items-center justify-between">
                        <h3 className="text-lg font-bold text-gray-900">이번 주 수업 일정</h3>
                        <Link href="/schedule" className="text-sm font-medium text-gray-500 underline decoration-gray-300 hover:text-gray-900">
                            전체 보기
                        </Link>
                    </div>
                    ...
                    <div className="space-y-4">
                        {schedules.map((item, i) => (
                            <div key={i} className="flex items-center justify-between rounded-xl bg-gray-50 p-5 hover:bg-gray-100 transition-colors group">
                                <div className="flex items-center gap-8">
                                    <span className="rounded-md bg-gray-200 px-3 py-1.5 text-sm font-bold text-gray-700 min-w-[80px] text-center">
                                        {item.date.slice(5).replace('-', '.')}
                                    </span>
                                    <span className="font-bold text-gray-900 text-lg">{item.time.split('-')[0]}</span>
                                    <div className="flex items-center gap-3">
                                        <span className="text-sm text-gray-600">{item.class.tutorName} 선생님</span>
                                        <span className="text-sm font-medium text-gray-900">{item.class.title}</span>
                                    </div>
                                </div>
                                <div className="flex gap-4 text-gray-400">
                                    ...
                                    <div className="cursor-pointer transition-opacity hover:opacity-80">
                                        <Image src="/google_note.svg" alt="Note" width={24} height={24} />
                                    </div>
                                </div>
                            </div>
                        ))}
